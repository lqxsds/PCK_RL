        ...
        with torch.device(self.device):
            ...
            for i in range(self.num_envs):
                ...
                builder.add_multiple_convex_collisions_from_file(
                    filename='/home/lqx/ManiSkill-3.0.0b15/mani_skill/envs/tasks/tabletop/kong.obj',
                    density=1000,  # 设置为动态物体
                    decomposition="coacd",
                    decomposition_params={
                        "threshold": 0.05,  # 增加阈值，降低精度要求
                        "max_convex_hull": 16,  # 减少凸包数量
                        "resolution": 512,  # 降低分辨率
                        "preprocess_resolution": 256,
                        "mcts_nodes": 15,  # 减少节点数
                        "mcts_iterations": 100,  # 减少迭代次数
                        "preprocess_mode": True,  # 关键：启用预处理模式
                    },
                    scale=(3,3,3)
                )
                builder.add_visual_from_file(filename='/home/lqx/ManiSkill-3.0.0b15/mani_skill/envs/tasks/tabletop/kong.obj',
                                             scale=(3,3,3))
                ...
                scene_idxs = [i]
                builder_2 = self.scene.create_actor_builder()
                builder_2.set_scene_idxs(scene_idxs)
                builder_2.add_multiple_convex_collisions_from_file(
                    filename='/home/lqx/ManiSkill-3.0.0b15/mani_skill/envs/tasks/tabletop/xiao.obj',
                    density=1000,  # 设置为动态物体
                    decomposition="coacd",
                    decomposition_params={
                        "threshold": 0.03,
                ...
                builder_2.add_visual_from_file(
                    filename='/home/lqx/ManiSkill-3.0.0b15/mani_skill/envs/tasks/tabletop/xiao.obj',scale=(3,3,3))
                builder_2.set_mass_and_inertia(mass=1, inertia=[0, 0, 0], cmass_local_pose=sapien.Pose())

                # builder.physx_body_type = "static"
                mesh3 = builder_2.build(name=f"hammer_2{i}")
                peg_2 = mesh3
                peg_2.set_collision_group_bit(group=2, bit_idx=31, bit=1)
                ###############################################################################################
                pegs_2.append(peg_2)
                self.remove_from_state_dict_registry(peg_2)
            ...
            self.peg_2 = Actor.merge(pegs_2, "peg_2")
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
          ...
          0.0400])
            ...
            self.agent.robot.set_pose(sapien.Pose([-0.615, 0.5, 0]))
            ...
            # 设置peg_2在peg正上方10cm处
            xyz_2 = xyz.clone()  # 复制peg的位置
            xyz_2[..., 2] += 1.0  # 在Z轴方向增加10cm
            obj_pose_2 = Pose.create_from_pq(p=xyz_2, q=q)
            self.peg_2.set_pose(obj_pose_2)
